...
#
# المعماريات: python:3.11-slim صورة متعددة المعماريات، والصورة تُبنى على
# x86_64 و arm64 معًا. جميع الاعتماديات لها عجلات جاهزة لـ aarch64 عدا
# pyzk، وهو بايثون خالص بلا شيفرة أصلية فيُبنى على أي معمارية. لذلك
# تُبنى الصورة على خادم ARM (مثل Ampere A1) دون أي تعديل هنا.
...
# لا EXPOSE هنا: كلُّ خدمةٍ تعلن منفذَها في compose (`expose:`).
#
# الصورةُ واحدة للخدمتين، و`EXPOSE 5000 8081` كان يجعل حاوية البصمة تعلن
# المنفذين معًا. وTraefik (وسيط Coolify) حين لا يُعطى منفذًا يختار
# **الأصغر** — 5000 — فيُرسل /iclock إلى hr_adms على منفذٍ لا يسمع عليه
# أحد، ويسكت الجهاز. والويب يعمل مصادفةً لأن 5000 منفذُه فعلًا.
