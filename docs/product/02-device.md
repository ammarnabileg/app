...
**لا تكتب منفذًا مثل 8081 أو 5000** — تلك منافذ داخلية عندنا ولا تُستعمل
من الجهاز.

### وشرطٌ على المستضيف لا على الجهاز

كان هنا مكتوبًا أننا «نستقبل على 80 ونحوّل إلى الاتصال المشفّر
داخليًّا». وهذا غير صحيح على كل تركيب، وعدمُ صحّته يظهر كجهازٍ لا
يعمل بلا سبب ظاهر — فقد وقع فعلًا عند عميل.

المستضيف قد يكون مضبوطًا على تحويل كل ما يأتي على 80 إلى HTTPS
بردّ `301`. وذلك صحيح للمتصفّحات ويتبعه المتصفّح بلا أن يشعر أحد.
**أما جهاز البصمة فلا يتبعه**: أكثر الموديلات لا تتكلّم TLS
إطلاقًا. فيرسل الجهاز، ويصله التحويل، ويسكت — ولا يظهر في النظام
أثرٌ لمحاولته، لأن الطلب لم يصل النظام أصلًا بل ردّ عليه المستضيف.

فحصُها بأمرٍ واحد من أي جهاز — **بـ`http` لا `https`**، فهذا ما
يفعله الجهاز:

```
curl -i "http://<عنوان-نظامك>/iclock/cdata?SN=healthcheck"
```

| الردّ | المعنى |
|---|---|
| `200` وجسمه `OK` | المسار سليم، والجهاز يستطيع الوصول |
| `301` أو `302` إلى `https` | **هذا هو المنع.** المستضيف يحوّل، والجهاز لا يتبع |
| `404` | النظام لا يستقبل `/iclock` على هذا العنوان |

ورفعُ المنع باستثناء `/iclock` من التحويل وحده — لا بإلغاء التحويل
كلّه: بقيّة النظام يفتحه بشر بمتصفّحات، وتشفيرُه واجب. في Apache:

```apache
RewriteEngine On
RewriteCond %{HTTPS} off
RewriteCond %{REQUEST_URI} !^/iclock
RewriteRule ^(.*)$ https://%{HTTP_HOST}%{REQUEST_URI} [R=301,L]
```

ويبقى `/iclock` على HTTP غير مشفّر. وهذا ليس تهاونًا نختاره: بروتوكول
ZKTeco نفسه غير مشفّر وغير موثَّق بمفتاح — يرسل الرقم التسلسلي في سطر
العنوان. فالتشفير هنا لم يكن موجودًا في أي حال، والنظام يحمي نفسه
بأن الجهاز غير المسجَّل تُرفض بياناته.
...
| الحالة | ابدأ من |
|---|---|
| الجهاز لا يظهر إطلاقًا | **ابدأ بأمر `curl` أعلاه بـ`http`.** ردُّ `301` يعني أن المستضيف يحوّل والجهاز لا يتبع — وهو منعٌ لا يظهر منه أثرٌ في النظام. ثم تأكّد من العنوان حرفًا بحرف، وأن المنفذ `80`، وأن `Enable Domain Name` مفعَّل |
| يظهر في «الأجهزة المكتشفة» ولا يُنفَّذ له أمر | اسم الجهاز في النظام يجب أن يحمل رقمه التسلسلي. والنظام يثبّته تلقائيًّا عند أول اتصال، فإن لم يحدث فابحث في السجلّ عن `ADMS unknown device` — السطر يحمل الرقم كما أرسله الجهاز |
