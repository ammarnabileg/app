...
def _check_photo():
    ...
    # العمر لا الوجود: كاميرا التطبيق الأصلية تُخرج EXIF دائمًا، فرفضُ
    # كل ما يحمله كان يرفض التطبيق نفسه. والسؤال الذي يهمّ: متى
    # التُقطت؟ فما مضى عليه أكثر من الحدّ يُردّ، وما لا تاريخ له يُقبل
    # ويحرسه الرمز والنطاق.
    age = field.photo_age_seconds(raw)
    if age is not None and age > field.PHOTO_FRESH_SECONDS:
        return None, (f'الصورة ليست جديدة — التُقطت قبل {int(age // 60)} دقيقة. '
